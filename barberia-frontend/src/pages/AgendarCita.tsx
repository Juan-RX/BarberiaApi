...
const AgendarCita = () => {
  ...
  const cargarDiasDisponibles = async () => {
    ...
    setLoading(true);
    try {
      // Calcular próximas 3 semanas (21 días)
      const hoy = new Date();
      hoy.setHours(0, 0, 0, 0);
      const fechaFin = new Date(hoy);
      fechaFin.setDate(fechaFin.getDate() + 21); // 3 semanas
    ...
    try {
      ...
      setMensaje({
        ...
        texto:
          ...
          error.response?.data?.message ||
          'El banco rechazó la transacción. Verifica los datos e intenta de nuevo.',
      });
    } finally {
